...
export function PublicationsSection({ isMobile, theme }: PublicationsSectionProps) {
  ...
  const filteredPublications = data.publications.filter(
    (publication) =>
      ...
      publication.year.toString().includes(searchQuery) ||
      publication.type.toLowerCase().includes(searchQuery.toLowerCase()),
  )
  ...
  return (
    ...
    >
      <div className="container mx-auto">
        ...
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6 sm:mb-8 theme-gradient-text">Publications</h2>
          ...
          >
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
              ...
              <Button
                onClick={() => window.open("https://orcid.org/0000-0002-8070-5400", "_blank")}
                variant="outline"
                className="w-full sm:w-auto theme-button-secondary flex items-center gap-2 px-4 py-2 text-sm font-medium transition-all duration-200 hover:scale-105"
              >
              ...
              <Button
                onClick={() => window.open("https://scholar.google.com/citations?user=63zBs60AAAAJ&hl=en", "_blank")}
                variant="outline"
                className="w-full sm:w-auto theme-button-secondary flex items-center gap-2 px-4 py-2 text-sm font-medium transition-all duration-200 hover:scale-105"
              >
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
            ...
            <p className="text-xs text-center mt-3 px-4" style={{ color: "var(--theme-text-secondary)" }}>
        ...
        {filteredPublications.length > 0 ? (
          <PublicationTimeline filteredPublications={filteredPublications} isMobile={isMobile} theme={theme} />
        ) : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
            <p className="text-lg" style={{ color: "var(--theme-text-secondary)" }}>
              No publications found matching "{searchQuery}"
            </p>
            <Button onClick={() => setSearchQuery("")} variant="outline" className="mt-4 theme-button-secondary">
              Clear Search
            </Button>
          </motion.div>
        )}
