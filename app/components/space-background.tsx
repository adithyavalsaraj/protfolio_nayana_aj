...
const MemoizedStar = React.memo(
  ({ star, layerY }: { star: Star; layerY: any }) => (
    <motion.div
      className="star"
      style={{
        left: `${star.x}%`,
        ...
        boxShadow:
          star.layer > 1 ? `0 0 ${star.size * 2}px ${star.color}40` : undefined,
        y: layerY,
      }}
      animate={{
        opacity: [
          star.opacity,
          star.opacity *
            (star.layer === 1 ? 0.2 : star.layer === 2 ? 0.3 : 0.4),
          star.opacity,
        ],
      ...
      transition={{
        ...
        delay:
          Math.random() * (star.layer === 1 ? 4 : star.layer === 2 ? 3 : 2.5),
      }}
    />
  )
);
...
export function SpaceBackground() {
  ...
  // Transform hooks
  const deepSpaceY = useTransform(scrollY, [0, 4000], [0, 0]);
  ...
  const drawMathematicalFormulas = useCallback(
    (ctx: CanvasRenderingContext2D, width: number, height: number) => {
      const formulas = [
  ...
  const drawOrbitalPaths = useCallback(
    (ctx: CanvasRenderingContext2D, width: number, height: number) => {
      const orbits = [
        {
          x: width * 0.2,
  ...
  const getPlanetBoxShadow = (type: Planet["type"], size: number) => {
    const shadows = {
      earth: `inset -${size * 0.12}px -${size * 0.12}px ${
        size * 0.25
      }px rgba(0, 0, 0, 0.6), inset ${size * 0.05}px ${size * 0.05}px ${
        size * 0.15
      }px rgba(255, 255, 255, 0.2), 0 0 ${
        size * 0.4
      ...
      neptune: `inset -${size * 0.14}px -${size * 0.14}px ${
        size * 0.28
      }px rgba(0, 0, 0, 0.6), inset ${size * 0.06}px ${size * 0.06}px ${
        size * 0.18
      }px rgba(135, 206, 235, 0.4), 0 0 ${
        size * 0.35
  ...
  if (theme === "light") {
    return (
      <div className={`space-background ${theme}-theme`}>
        ...
        <motion.div
          className="animated-elements"
          style={{ y: lightBgParallax }}
        >
          ...
          >
            ...
            {planet.type === "saturn" && (
              <div className="saturn-rings rings-back">
                <div
                  ...
                  style={getRingStyles(planet, "ring-1")}
                />
                <div
                  className="ring ring-2"
                  style={getRingStyles(planet, "ring-2")}
                />
                <div
                  className="ring ring-3"
                  style={getRingStyles(planet, "ring-3")}
                />
              </div>
            )}
            ...
            {planet.type === "saturn" && (
              <div className="saturn-rings rings-front">
                <div
                  className="ring ring-1 front"
                  style={getRingStyles(planet, "ring-1")}
                />
                <div
                  className="ring ring-2 front"
                  style={getRingStyles(planet, "ring-2")}
                />
                <div
                  className="ring ring-3 front"
                  style={getRingStyles(planet, "ring-3")}
                />
              </div>
            )}
